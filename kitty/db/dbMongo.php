...
class dbMongo { 
	...
	public function __construct($host=null, $persistent = false){
		if(!class_exists("\Mongo",false))
			throw new \kitty\base\ExceptionError("php_mongo расширение не установлен");			
		try{	
	...
	**/
	...
	public function selectDB($name = null){
		...
		try{
			...
			return $this;
		}catch(\kitty\base\ExceptionError $e){
			throw $e;
		}
	}
	...
	public function getDb(){
		...
		if(!$result){
			throw new \kitty\base\ExceptionError('Не выбрана MongoDB');
		}
		return $result;
	...
	public function execute($code, $params = array() ){
		...
		if(!$result['ok']){
			throw new \kitty\base\ExceptionError("Error: {error}\nCode: {js}", array(
				'{error}'=> $result['errmsg'],
				'{js}'=> $code,
			));		
		}
